...
import { WarzoneDashboard } from "./WarzoneDashboard";
...
function App() {
  ...
  const [clanCreado, setClanCreado] = useState(false)
    ...
    try {
      ...
      if (esLectura) {
        ...
        if (params.funcion === "retornar_todo"){
          cambiarRespuesta(`🎮 Soldier: ${decoded[4]}\n📅 Season Joined: ${decoded[0]}\n⭐ Rank Tier: ${decoded[3]['raw'][1]}% XP Boost\n🌍 Region: ${decoded[1]}`)
        }
        return decoded;
      }
      ...
      alert("Tango Down: " + error.message);
      console.error(error);
    } finally {
      cambiarEstado(false);
    }
  ...
  return (
    <div className="app-container">
      <header className="warzone-header">
        <div className="brand">
          <div className="brand-logo">🎯</div>
          <div className="brand-text">
            <h1>WARZONE COMMAND</h1>
            <div className="subtitle">SQUAD MANAGEMENT SYSTEM</div>
          </div>
        </div>
        <ConnectButton />
      </header>
      ...
      <main className="main-content">
        {!cuenta ? (
          <div className="hero-section">
            <h1 className="hero-title">WARZONE SQUAD MANAGER</h1>
            <p className="hero-subtitle">
              Gestiona tu escuadra, coordina estrategias y domina Verdansk 
              con tecnología blockchain descentralizada.
            </p>
